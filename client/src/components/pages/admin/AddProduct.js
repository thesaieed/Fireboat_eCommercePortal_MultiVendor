...
function AddProduct() {
  ...
  const [categories, setCategories] = useState([])
    ...
    const onFinish = async (values)=>{
        ...
        try {
            const response = await axios.post("http://localhost:5000/admin/addproduct",values)
  ...
  return (
    ...
    //class name card-signup header-solid h-full ant-card pt-0  mb-2
            ...
            title={<h5 style={{fontWeight:'bold',fontSize:'1.5rem'}}>Enter Product Details</h5>}
            bordered="false"
          >
      <Form
        form = {form}
    ...
    >
      <Select style={{fontWeight:'normal'}} placeholder = "Select a category">
        {categories.map((category) => (
          <Option key={categories.name} value={category.name}>
            {category.name}
          </Option>
        ))}
      </Select>
